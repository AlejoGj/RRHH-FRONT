...
export default function AspirantesTable({ refresh }) {
  ...
  const cargarAspirantes = async () => {
    try {
      setCargando(true);
      const response = await aspirantesService.getAll();
      if (response.success) {
        setAspirantes(response.data.sort((a, b) => new Date(b.fecha) - new Date(a.fecha)));
      }
    } catch (error) {
  ...
  const handleEliminar = async (id, nombre) => {
    if (
      ...
      try {
        setEliminando(id);
        const response = await aspirantesService.delete(id);
        if (response.success) {
          cargarAspirantes();
          mostrarMensaje(`${nombre} ha sido eliminado exitosamente`, "exito");
        } else {
          mostrarMensaje(response.message || "Error al eliminar el aspirante", "error");
        }
      } catch (error) {
        mostrarMensaje(error.message || "Error al eliminar el aspirante", "error");
        console.error(error);
      } finally {
        setEliminando(null);
      }
                ...
                <tr key={aspirante.id} className="tabla-fila">
                  ...
                  <td className="acciones-celda">
                    <button
                      ...
                      disabled={eliminando === aspirante.id}
                      title="Eliminar aspirante"
                    >
                      {eliminando === aspirante.id ? '⏳ Eliminando...' : '🗑️ Eliminar'}
                    </button>
                  </td>
                </tr>
              ))}
