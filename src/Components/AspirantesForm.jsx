...
export default function AspirantesForm({ onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Crear aspirante
    try {
      const response = await aspirantesService.create({
      ...
      if (response.success) {
        ...
        mostrarMensaje(response.message || "Error al registrar el aspirante", "error");
      }
    } catch (error) {
      mostrarMensaje(error.message || "Error al registrar el aspirante", "error");
      console.error(error);
    } finally {
      setIsLoading(false);
    }
